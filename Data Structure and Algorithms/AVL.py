...
class AVLTree:
    ...
    def insert_node(self, data, node):
        ...
        if data < node.data:                                             # we have to go to the left subtree
            if node.left_node:
        ...
        else:                                                            # we have to visit the right subtree
            if node.right_node:
    ...
    def remove_node(self, data, node):                           #same as binary trees (print statments will help though)
    ...
    def rotate_right(self, node):
        ...
        temp_left_node = node.left_node                              #AV_tree_diagram.png
        t = temp_left_node.right_node

        temp_left_node.right_node = node                             #given C's address to D
        node.left_node = t                                           #(rotation done)

        if t is not None:
            t.parent = node

        temp_parent = node.parent                                    #telling the parents that the children has been changed
        node.parent = temp_left_node
        temp_left_node.parent = temp_parent

        if temp_left_node.parent is not None and temp_left_node.parent.left_node == node:
            temp_left_node.parent.left_node = temp_left_node         #giving parent the location of B
